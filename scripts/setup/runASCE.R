# runASCE.R
# nolint start
# Builds ASCETable (MCER + DESIGN spectra from UHSTable). Idempotent.

if (!exists("ASCETable", envir = .GlobalEnv) || !nrow(get("ASCETable", envir = .GlobalEnv))) {

## -----------------------------------------------------------------
##  build MCER spectra (TR = 2475) for both “gem” and “gmdp”
## -----------------------------------------------------------------
#DT1 <- UHSTable[ID == "max" & p == "mean",
DT1 <- UHSTable[ p == "mean",
                {
