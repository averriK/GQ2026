...
By explicitly accounting for uncertainty in the seismic hazard and in the site-amplification response, the methodology delivers mean and quantile estimates of the seismic coefficient. Applied to a `r SUMMARY$Hs` m embankment on glaciolacustrine foundation at $T_R = `r SUMMARY$trLabel`$ yr, the procedure yields $\bar{k}_{\max} = `r SUMMARY$DaRows[[2]]$kmaxMean`$ ($K_h = `r SUMMARY$DaRows[[2]]$khMean`\%$) at a target displacement of `r SUMMARY$DaRows[[2]]$Da` cm.
...
En tenant explicitement compte des incertitudes dans l'aléa sismique et dans la réponse d'amplification de site, la méthodologie fournit des estimations moyennes et quantiles du coefficient sismique. Appliquée à une digue de `r SUMMARY$Hs` m sur fondation glaciolacustre pour $T_R = `r SUMMARY$trLabel`$ ans, la procédure donne $\bar{k}_{\max} = `r SUMMARY$DaRows[[2]]$kmaxMean`$ ($K_h = `r SUMMARY$DaRows[[2]]$khMean`\%$) pour un déplacement cible de `r SUMMARY$DaRows[[2]]$Da` cm.
